...
def render_khbd_section(run_ai_prompt_safe_func):
    ...
    with tab_xay_dung:
        ...
        # Thay thế khung cũ bằng ô văn bản "Yêu cầu ràng buộc khác" làm căn cứ cho AI
        ...
        if not ten_bai:
            ...
            with st.spinner("🧠 Trợ lý AI đang nghiên cứu kỹ dữ liệu nguồn đa file và tiến hành lập tiến trình bài dạy..."):
                ...
                2. ĐỘ CHÍNH XÁC & ĐỊNH DẠNG:
                   - { 'Bám sát hoàn toàn 100% nội dung kiến thức từ tài liệu nguồn dưới đây. Không tự chế thuật ngữ, định lý ngoài nguồn.' if uati_bam_sat else '' }
                   ...
                   - { 'HÌNH ANH MINH HỌA: Tại các bước lý thuyết phù hợp, hãy chèn chính xác dòng chữ dòng đơn là "[Hình ảnh minh họa]" để hệ thống nhúng ảnh từ file gốc.' if danh_sách_ảnh else '' }
                
                3. { 'NĂNG LỰC SỐ VÀ AI: Thiết kế các hoạt động yêu cầu học sinh làm việc trên máy tính, khai thác học liệu số hoặc sử dụng công cụ AI trợ giúp giải bài tập.' if tich_hop_ai else '' }
                ...
                # Lưu trực tiếp vào Session State và gọi hàm render lại nội dung ngay lập tức
