...
def render_personal_plan():
    ...
    t_name = col_t.text_input("Họ và tên Giáo viên giảng dạy:", placeholder="Ví dụ: Thầy Lê Hồng Dưỡng")
    s_name = col_s.selectbox("Môn học / Phân môn phụ trách:", ["Khoa học tự nhiên (Vật lý)", "Khoa học tự nhiên (Sinh học)", "Khoa học tự nhiên (Hóa học)", "Toán học", "Ngữ văn", "GDTC"])
    
    col_g, col_w = st.columns(2)
    grade_target = col_g.text_input("Khối lớp phân công dạy:", placeholder="Ví dụ: Khối 7, Khối 8")
    week_count = col_w.text_input("Tổng số tuần thực hiện kế hoạch:", placeholder="Ví dụ: 35 Tuần")
    
    st.markdown("**💬 Các tiêu chí đặc thù hoặc lưu ý phân bổ tiết (Nếu có):**")
    note_plan = st.text_area("Yêu cầu bổ sung cho AI:", placeholder="Ví dụ: Phân bổ chi tiết số tiết cho chương Tốc độ ở vật lý 7 học kỳ I...", label_visibility="collapsed")
    
    col_btn, _ = st.columns([1.5, 2.5])
    with col_btn:
        run_ai_plan = st.button("🧠 Khởi tạo Kế hoạch bằng AI", type="primary", use_container_width=True)
    ...
    if run_ai_plan:
        if not t_name or not grade_target:
            ...
            with st.spinner("Trợ lý AI đang lập tiến trình phân bổ tiết Phụ lục III..."):
                ...
                CẤU TRÚC PHẢI BAO GỒM ĐỦ CÁC MỤC CHUẨN:
                I. KẾ HOẠCH DẠY HỌC PHÂN PHỐI CHƯƠNG TRÌNH
                   - Phân bổ rõ ràng tên Chương/Bài học, số tiết, tuần thực hiện phù hợp với phân môn {s_name}.
                II. CÁC NHIỆM VỤ KHÁC ĐƯỢC GIAO (Bồi dưỡng học sinh giỏi, Phụ đạo học sinh yếu, Sinh hoạt tổ chuyên môn).
                
                YÊU CẦU ĐỊNH DẠNG: Văn bản chi tiết đầy đủ chữ, dùng dấu gạch ngang '-' đầu mục, KHÔNG DÙNG dấu sao kép '**' để tránh lỗi font chữ đậm.
                LƯU Ý RIÊNG: {note_plan}
                """
                try:
                    from app import run_ai_prompt_safe
                    res_plan, _ = run_ai_prompt_safe(prompt_plan, st.secrets.get("GEMINI_API_KEY", ""))
    ...
    with st.container(border=True):
        if st.session_state["ai_plan_output"]:
            ...
            # Nút tải file Word Phụ lục III chuẩn chỉ hành chính
