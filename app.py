...
def export_to_docx_vietnam_standard(text_content, title_name, school_name="TRƯỜNG THCS NGUYỄN CHÍ THANH", group_name="TỔ KHOA HỌC TỰ NHIÊN - GDTC"):
    ...
    # BỘ XỬ LÝ KHẮC PHỤC LỖI KẺ BẢNG & ĐỊNH DẠNG DÒNG
    in_table = False
    table_data = []
    ...
    def build_table():
        ...
        table.style = 'Table Grid' # KHẮC PHỤC TRIỆT ĐỂ MẤT VIỀN BẢNG
        for r_idx, row in enumerate(table_data):
            for c_idx, cell_val in enumerate(row):
                if c_idx < cols:
                    # Loại bỏ markdown in đậm trong bảng để tránh lỗi hiển thị
            ...
            if st.button("🛠️ Thiết lập Đề thi + Ma trận & Đặc tả"):
                if not api_key_input: st.error("Thầy cần cấu hình Gemini API Key tại thanh bên!")
                    ...
                    with st.spinner("AI đang xử lý Ma trận, Đặc tả và Đề thi..."):
                        try:
                            # KHẮC PHỤC LỖI KHÔNG BÁM SÁT FILE & KHÔNG SINH MA TRẬN
...
PHẦN 3. ĐỀ KIỂM TRA VÀ ĐÁP ÁN CHI TIẾT (Chia rõ Trắc nghiệm và Tự luận)"""
            ...
            if not st.session_state["db_de_kiem_tra"]:
                ...
                for idx, item in enumerate(st.session_state["db_de_kiem_tra"]):
                    with st.expander(f"📋 {item['ten_de']} - Môn: {item['mon']} ({item['khoi']})"):
                        ...
                        # BỔ SUNG CHỨC NĂNG XÓA ĐỀ KIỂM TRA
                        with col_bt2:
    ...
    # PHÂN HỆ QUẢN LÝ (Giữ nguyên các tab)
    if chuc_nang_chinh == "1. Hệ thống Quản lý và Phân công chuyên môn giảng dạy":
        ...
        # Giao diện y hệt phiên bản gốc, bạn có thể copy vào đây để đảm bảo độ dài
        st.info("Tính năng biên bản hoạt động bình thường...")
        ...
        if st.button("🪄 AI Thiết kế Phụ lục III chuẩn 5512"):
            with st.spinner("AI đang xử lý..."):
                res, _ = run_ai_prompt_safe(f"Thiết kế phụ lục III 5512 cho môn {mon_tu_dong}", api_key_input)
                st.markdown(res)
                st.download_button("📥 Tải Phụ lục III", data=export_to_docx_vietnam_standard(res, f"Phu_luc_III_{gv_selected}"), file_name=f"Phu_luc_III_{gv_selected}.docx")
