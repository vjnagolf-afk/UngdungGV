...
def render_stem_section():
    ...
    with tab1:
        render_tab_1()
    with tab2:
        # (Thầy giữ nguyên các phần import và render_tab_1() ở trên nhé)
    ...
    if st.button("🚀 Kích hoạt AI thiết kế Kế hoạch bài dạy (Thẻ 2)", type="primary", use_container_width=True, key="btn_ai_t2"):
        if not ten_chu_de_t2:
            ...
            with st.spinner("Hệ thống đang phân tích sư phạm và thiết kế KHBD chi tiết. Quá trình này có thể mất ít phút..."):
                ...
                # --- XÂY DỰNG PROMPT CHUYÊN SÂU GỬI CHO AI ---
                ...
                try:
                    # TẠI ĐÂY LÀ NƠI THẦY GỌI API THẬT
                    # Ví dụ với google.genai:
                    # client = genai.Client() # Giả sử thầy đã khởi tạo client
                    # response = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
                    # noi_dung_ai = response.text
                    
                    # Tạm thời em mô phỏng nội dung AI trả về sau khi nhận Prompt dài để thầy hình dung (Khi ghép API thật thầy xóa đoạn giả lập này đi):
...
"""
    ...
    if st.session_state.stem_generated_content != "":
        ...
        col_download, col_save = st.columns(2)
        with col_download:
            # Đổi tên file xuất ra thành KHBD thay vì Giao_an
            docx_file = create_word_file(ten_chu_de_t2, st.session_state.stem_generated_content)
            st.download_button(
