...
def render_exam_designer_section(api_key_input, run_ai_prompt_safe_func):
    ...
    with tab_thiet_ke:
        ...
        # XỬ LÝ TẠO ĐỀ VÀ ẨN KẾT QUẢ Ở TAB 1
        if btn_tao:
...
4. Về Đồ thị/Hình vẽ: AI không thể vẽ ảnh, do đó hãy để lại dòng ghi chú [DÀNH KHOẢNG TRỐNG TẠI ĐÂY ĐỂ GIÁO VIÊN CHÈN ĐỒ THỊ/HÌNH ẢNH] kèm mô tả chi tiết trục tọa độ để giáo viên tự chèn vào file Word sau.
...
PHẦN 3. ĐỀ KIỂM TRA & ĐÁP ÁN"""
                        ...
                        st.success("🎉 Đã tạo đề thi thành công! Thầy/Cô vui lòng chuyển sang thẻ 📂 THƯ MỤC ĐỀ ĐÃ XÂY DỰNG để xem, thu gọn và tải file Word.")
    ...
    # THƯ MỤC LƯU TRỮ VÀ XUẤT KẾT QUẢ ĐÃ ĐƯỢC CHUYỂN SANG ĐÂY
    with tab_kho_luu_tru:
        ...
        if not st.session_state["db_de_kiem_tra"]:
            ...
            for idx, item in enumerate(reversed(st.session_state["db_de_kiem_tra"])):
                ...
                # Tính năng thu gọn đề (st.expander)
