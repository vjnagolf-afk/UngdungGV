...
def render_exam_designer_section(api_key_input, run_ai_prompt_safe_func):
    ...
    with tab_thiet_ke:
        ...
        # NÂNG CẤP HỖ TRỢ TẢI NHIỀU FILE CÙNG LÚC
        uploaded_files_de = st.file_uploader(
            "Tải lên CÁC file giới hạn kiến thức hoặc bộ đề thi làm căn cứ (.pdf, .docx):", 
            type=["pdf", "docx"], 
            key="up_file_de_thi", 
        ...
        if uploaded_files_de:
            for file in uploaded_files_de:
                content_de_nguon += f"\n--- NỘI DUNG TỪ TÀI LIỆU: {file.name} ---\n"
                if file.name.endswith('.docx'):
                    content_de_nguon += read_uploaded_docx(file)
                else:
                    content_de_nguon += read_uploaded_pdf(file)
            st.success(f"✅ Đã nạp thành công {len(uploaded_files_de)} tài liệu kiến thức nguồn vào bộ nhớ AI.")
            
        uu_tien_de = st.checkbox("🎯 BÁM SÁT 100% CÁC TÀI LIỆU/KIẾN THỨC NGUỒN VÀ BẮT BUỘC TẠO MA TRẬN", value=True)
        ...
        if st.button("🛠️ Thiết lập Đề thi + Ma trận & Đặc tả"):
            if not api_key_input: st.error("Thầy cần cấu hình Gemini API Key tại thanh bên!")
                ...
                with st.spinner("AI đang xử lý Ma trận, Đặc tả và Đề thi từ các nguồn tài liệu..."):
                    try:
...
"""
                        if uu_tien_de and content_de_nguon:
                            prompt_de += f"""\n\nBẮT BUỘC BÁM SÁT TUYỆT ĐỐI 100% NỘI DUNG TỔNG HỢP TỪ CÁC TÀI LIỆU NGUỒN SAU ĐÂY MỚI ĐƯỢC RA ĐỀ:
--- BẮT ĐẦU DỮ LIỆU NGUỒN TỔNG HỢP ---
{content_de_nguon}
--- KẾT THÚC DỮ LIỆU NGUỒN TỔNG HỢP ---"""

                        prompt_de += """\n
LƯU Ý QUAN TRỌNG VỀ ĐỊNH DẠNG (BẮT BUỘC TUÂN THỦ):
1. TUYỆT ĐỐI KHÔNG sử dụng định dạng toán học LaTeX (tuyệt đối KHÔNG dùng ký tự $ hay $$).
2. Viết công thức hóa học và toán học bằng văn bản thường, kết hợp thẻ HTML cho chỉ số:
   - Chỉ số dưới dùng thẻ <sub>...</sub> (Ví dụ nước là: H<sub>2</sub>O, Axit là H<sub>2</sub>SO<sub>4</sub>)
   - Chỉ số trên dùng thẻ <sup>...</sup> (Ví dụ mét vuông là: m<sup>2</sup>, x bình phương là: x<sup>2</sup>)
   - Phân số viết dạng a/b.
        ...
        if not st.session_state["db_de_kiem_tra"]:
            ...
            for idx, item in enumerate(st.session_state["db_de_kiem_tra"]):
                with st.expander(f"📋 {item['ten_de']} - Môn: {item['mon']} ({item['khoi']})"):
                    ...
                    with col_bt1:
                        st.download_button(label="📥 Tải File Word (Chuẩn công thức Hóa/Toán)", data=export_to_docx_vietnam_standard(item["noi_dung"], item["ten_de"]), file_name=f"{item['ten_de']}_{idx}.docx", key=f"dl_de_thi_{idx}")
                    with col_bt2:
