    ...
    with tab_thiet_ke:
        ...
        with col_main2:
            ...
            st.markdown(
                ...
                f"<div class='lbl-green-solid'>ĐIỂM: <span class='val-field'>{tong_diem_tl:.1f}</span></div>"
        ...
        if run_exam_ai:
            ...
            prompt_exam = (
                ...
                f"- Trắc nghiệm tổng cộng {tong_cau_tn} câu với {tong_diem_tn} điểm, chi tiết gồm: Nhiều lựa chọn: {c1} câu, Đúng/sai: {c2} câu, Điền khuyết: {c3} câu, Trả lời ngắn: {c4} câu.\n"
                f"- Tự luận tổng cộng {num_tl_input} câu lớn với tổng điểm là {tong_diem_tl} điểm (Điểm chi tiết từng câu con: {', '.join([f'Câu {i+1}: {s}đ' for i, s in enumerate(tl_scores)])}).\n"
