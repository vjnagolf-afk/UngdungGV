...
def render_exam_designer_section(api_key_input, run_ai_prompt_safe_func):
    ...
    with tab_thiet_ke:
        ...
        # --- 💥 CỘT LỆCH PHẢI: PHẦN TỰ LUẬN (ĐÃ MỞ KHÓA HOÀN TOÀN BIẾN KEY) ---
        with col_main2:
            st.markdown("<div class='title-green-header'>PHẦN TỰ LUẬN</div>", unsafe_allow_html=True)
            
            # Khởi tạo giá trị tạm thời trong session_state để bộ đếm tự do nhảy số
            if "num_tl_unlocked_val" not in st.session_state:
                st.session_state["num_tl_unlocked_val"] = 4
                
            # 🌟 MỞ KHÓA TRIỆT ĐỂ: Thay đổi key độc nhất độc bản, không bị ghi đè lặp bởi luồng cũ
            num_tl_input = st.number_input("TỔNG SỐ CÂU TỰ LUẬN:", min_value=0, max_value=20, value=st.session_state["num_tl_unlocked_val"], step=1, key="key_unlocked_num_tl_perfect_v10")
            ...
            # Xuất hộp tổng điểm động chạy xuôi dòng tự nhiên, triệt tiêu lỗi lề âm đè chữ
            st.markdown(
